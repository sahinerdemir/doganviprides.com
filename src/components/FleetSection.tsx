...
import { Users, Briefcase, ArrowUpRight, Sparkles } from "lucide-react";
        ...
        <Reveal>
          <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-16 gap-4">
            <div className="space-y-2">
              <div className="inline-flex items-center space-x-1.5 text-[11px] font-bold text-gold-400 uppercase tracking-widest">
            ...
            <p className="text-xs sm:text-sm text-zinc-400 max-w-md font-light">
              Brand-new 2026 showroom models, sanitized before every ride. Equipped with chilled water, 5G Wi-Fi, and executive privacy amenities.
            </p>
          </div>
        </Reveal>
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {FLEET_DATA.map((vehicle) => (
            <StaggerItem key={vehicle.id}>
              <div className="bg-[#101015] border border-white/5 rounded-3xl overflow-hidden hover:border-white/20 transition-all duration-500 flex flex-col justify-between group h-full">
                {/* Image */}
                <div className="relative h-64 sm:h-80 w-full overflow-hidden bg-black/40">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="px-3 py-1 rounded-full bg-black/80 backdrop-blur-md border border-gold-400/40 text-[10px] font-bold text-gold-300 uppercase tracking-wider">
                      {vehicle.year} Model
                    </span>
                  </div>
